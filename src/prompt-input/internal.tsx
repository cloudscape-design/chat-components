...
import { lineHeightBodyM, spaceScaledXxxs, spaceStaticXxs } from "@cloudscape-design/design-tokens";
...
import { convertAutoComplete } from "./utils";
...
const InternalPromptInput = forwardRef(
  (
    {
      value,
    ...
    const handleKeyDown = (event: KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) {
        ...
        fireNonCancelableEvent(onAction, { value: value || "" });
      }
    };
    ...
    const adjustTextareaHeight = useCallback(() => {
      if (textareaRef.current) {
        ...
        const minTextareaHeight = `calc(${LINE_HEIGHT} +  ${spaceScaledXxxs} * 2)`; // the min height of Textarea with 1 row
      ...
      <div className={clsx(styles["primary-action"], testutilStyles["primary-action"])}>
        {customPrimaryAction ?? (
          <Button
            ...
            onClick={() => fireNonCancelableEvent(onAction, { value: value || "" })}
            variant="icon"
            {...(readOnly && { nativeButtonAttributes: { "aria-disabled": "true" } })}
          />
        )}
      </div>
    );
