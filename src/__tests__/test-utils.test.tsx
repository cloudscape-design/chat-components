...
function getComponentSelectors(componentName: string) {
  ...
  // For this reason, casting the function names into the Avatar component.
  const findName = `find${componentNamePascalCase}` as "findAvatar";
  const findAllName = wrapperPropsList.find((selector) => findAllRegex.test(selector)) as "findAllAvatars";

  return { findName, findAllName };
}

describe.each(Object.keys(components))("ElementWrapper selectors for %s component", (componentName) => {
  ...
  describe("selectors wrapper", () => {
    ...
    test(`${findAllName} appends the specified selector to the default ${componentName} selectors`, () => {
      ...
      const wrapper = createWrapperSelectors();
      const firstItemSelector = wrapper[findAllName]("[data-type=second-type]").get(1).toSelector();
      const secondItemSelector = wrapper[findAllName]("[data-type=second-type]").get(2).toSelector();
      const firstElement = container.querySelector(firstItemSelector);
      const secondElement = container.querySelector(secondItemSelector);
